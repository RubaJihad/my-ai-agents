...
def clean_llm_output(output):
    """Strip markdown code blocks if present in LLM output."""
    output = output.strip()
...
def execute_write_action(db, generated_code):
    ...
    with a NameError.
    """
    generated_code = clean_llm_output(generated_code)
    local_vars = {}
    try:
...
def run_orchestrator_plan(db, original_request, plan_text):
    ...
    """
    plan_text = clean_llm_output(plan_text)
    try:
